#' @title get_tab_6_salud
#'
#' @description
#' Esta funcion permite organizar los datos del Cuadro 6  relacionado con el tema de salud
#' del Tomo III de los Resultados del Censo Nacional de 2017, el cual tiene el siguiente título:
#' " POBLACION CENSADA DE 12 Y MAS ANOS DE EDAD, POR AFILIACION A ALGUN TIPO DE SEGURO DE SALUD,
#'  SEGUN PROVINCIA, DISTRITO, AREA URBANA Y RURAL, SEXO Y ESTADO CIVIL O CONYUGAL"
...
#' @export
get_tab_6_salud <- function(file, dep_name = NULL){
  df <- readxl::read_xlsx(file,
    ...
    dplyr::rename(estado_civil = var_1) |>
    dplyr::relocate(estado_civil, .after = "tipo_seguro") |>
