#' @title get_tab_2_salud
#'
#' @description
#'
#' Esta función permite organizar los datos del Cuadro Nº 2  relacionado con el tema de salud
#' del Tomo II de los Resultados del Censo Nacional de 2017, el cual tiene el siguiente título:
#' "POBLACIÓN CENSADA, POR AFILIACIÓN A ALGÚN TIPO DE SEGURO DE SALUD, SEGÚN
#' PROVINCIA, DISTRITO, ÁREA URBANA Y RURAL,SEXO Y ALGUNA DIFICULTAD O LIMITACIÓN PERMANENTE".
#'
...
#' @export
get_tab_2_salud <- function(file, dep_name = NULL){
  df <- readxl::read_xlsx(file,
    ...
    dplyr::rename(limitacion = var_1) |>
    dplyr::relocate(limitacion, .after = "tipo_seguro") |>
